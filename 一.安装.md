...
## 目录
   - [安装条件](#安装条件)
   - [镜像下载](#OPENTHOS镜像下载)
   - [创建启动U盘](#创建OPENTHOS启动U盘)
      - [Windows用户制作方式](#Windows用户制作OPENTHOS启动U盘)
      - [Linux用户制作方式](#Linux用户制作OPENTHOS启动U盘)
   - [U盘安装](#通过U盘来安装OPENTHOS)
      - [自动安装](#自动安装)
      - [手动安装](#手动安装)
   - [启动界面](#启动界面)
...
## OPENTHOS镜像下载
   ...
   - 方式三：[OneDrive](https://jcmkh-my.sharepoint.com/:f:/g/personal/nx86022_office365home_vip/Ev5DTbq6VD5JhGhb3AJx8N8Bu2O611SdnCF7cGIolYJtmQ?e=AkThgk)
```
...
要在Windows环境下制作OPENTHOS启动U盘，您可以[使用UltraISO(软碟通)制作U盘启动盘](#ultraiso制作步骤)，也可以[手动提取到U盘](#手动提取到u盘)。
同时您的计算机还应满足以下条件：
...
#### 手动提取到U盘
1. 提取img文件到U盘（可以通过[7-zip](https://www.7-zip.org/)解压）  
此时U盘根目录有efi和OpenThos两个文件夹  
![](./pic/anzhuang/extractimg_1.png)
  
2. **将U盘名称改为OTO_INSTDSK**  
![](./pic/anzhuang/extractimg_2.png)  
...
## 启动界面
   - 正在启动OPENTHOS系统，初次安装启动时间较长，需要1到2分钟，平时启动大概需要几十秒钟。
   - 初次启动后进入[首次配置](三.首次配置.md)，以后启动后会进入[桌面](四.桌面.md)。  
![](pic/kaiji/start.png)
